          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label="Request a sample">
            ...
            >
              ...
              <div className="relative px-6 pt-6 pb-4 border-b border-stone-100">
                ...
                <h2 className="text-2xl font-semibold text-stone-900" style={{ fontFamily: "'Playfair Display', serif" }}>
                  Request a Sample
                </h2>
                ...
                <button onClick={handleClose} className="absolute top-5 right-5 p-2 rounded-lg text-stone-400 hover:text-stone-900 hover:bg-stone-50 transition-colors cursor-pointer" aria-label="Close modal">
              ...
              <AnimatePresence mode="wait">
                {step === "form" ? (
                  ...
                  >
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input id="s-name" type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })}
                          className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm text-stone-900 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors"
                    ...
                    <div>
                      ...
                      <select id="s-style" value={form.style} onChange={(e) => setForm({ ...form, style: e.target.value })}
                        className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm text-stone-900 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors bg-white cursor-pointer"
                    ...
                    <div>
                      ...
                      <textarea id="s-notes" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} rows={2}
                        className="w-full px-3 py-2 rounded-lg border border-stone-200 text-sm text-stone-900 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors resize-none"
                        style={{ fontFamily: "Inter, sans-serif" }} placeholder="Room size, color preferences, installation type…" />
                    </div>

                    <button type="submit" disabled={loading}
                      className="w-full py-3 rounded-full bg-stone-900 text-white text-sm font-semibold hover:bg-stone-800 disabled:opacity-60 transition-all cursor-pointer flex items-center justify-center gap-2"
                      style={{ fontFamily: "Inter, sans-serif" }}>
                      {loading ? (
                        <>
                  ...
                  >
                    ...
                    <h3 className="text-xl font-semibold text-stone-900 mb-2" style={{ fontFamily: "'Playfair Display', serif" }}>Sample on its way!</h3>
                    ...
                    <button onClick={handleClose}
                      className="px-6 py-2.5 rounded-full bg-stone-900 text-white text-sm font-medium hover:bg-stone-800 transition-colors cursor-pointer"
                      style={{ fontFamily: "Inter, sans-serif" }}>
                      Continue browsing
                    </button>
                  </motion.div>
