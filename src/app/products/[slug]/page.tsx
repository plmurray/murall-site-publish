      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="sticky top-28">
            {/* Main image */}
            <div
              className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-stone-100 mb-3 cursor-zoom-in"
            ...
            >
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-1.5">
                {product.isBestseller && <span className="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-amber-50 text-amber-800 border border-amber-200">Bestseller</span>}
                {product.isNew && <span className="px-2.5 py-1 rounded-full text-[10px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200">New</span>}
              </div>

              {/* Zoom hint */}
              <div className="absolute bottom-4 right-4">
                <span className="px-2.5 py-1 rounded-full bg-white/80 backdrop-blur-sm text-[10px] text-stone-600 border border-stone-200">Click to zoom</span>
              </div>
            </div>
              ...
              <div className="flex gap-2">
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setActiveImg(i)}
                    className={`relative w-20 aspect-square rounded-xl overflow-hidden border-2 transition-all cursor-pointer ${activeImg === i ? "border-stone-900" : "border-transparent hover:border-stone-300"}`}
          ...
          <div className="py-2">
            ...
            <h1 className="text-3xl sm:text-4xl font-semibold text-stone-900 mb-3" style={{ fontFamily: "'Playfair Display', serif" }}>{product.name}</h1>
            ...
            <div className="flex items-baseline gap-2 mb-6">
              <span className="text-3xl font-semibold text-stone-900" style={{ fontFamily: "'Playfair Display', serif" }}>${product.price}</span>
            ...
            {/* Install type */}
            <div className="flex items-center gap-2 mb-6 p-3 bg-stone-50 rounded-xl border border-stone-100">
            ...
            <div className="flex gap-3 mb-4">
              ...
              <button onClick={handleAddToCart}
                className={`flex-1 py-3 rounded-full text-sm font-semibold transition-all duration-200 cursor-pointer flex items-center justify-center gap-2 ${added ? "bg-emerald-600 text-white" : "bg-stone-900 text-white hover:bg-stone-800"}`}
            ...
            <button onClick={() => setSampleOpen(true)}
              className="w-full py-3 rounded-full border border-stone-200 text-stone-700 text-sm font-medium hover:border-stone-400 transition-colors cursor-pointer mb-6"
              style={{ fontFamily: "Inter, sans-serif" }}>
            ...
            <div className="flex flex-wrap gap-1.5 mb-8">
              {product.tags.map((tag) => (
                <span key={tag} className="text-[10px] px-2.5 py-1 rounded-full bg-stone-100 text-stone-500 border border-stone-200" style={{ fontFamily: "Inter, sans-serif" }}>{tag}</span>
              ))}
            </div>

            {/* Specifications */}
            <div className="border-t border-stone-100 pt-6">
              <h2 className="text-sm font-semibold text-stone-900 mb-4" style={{ fontFamily: "'Playfair Display', serif" }}>Product specifications</h2>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              {[
                ...
                <div key={label} className="text-center p-3 bg-stone-50 rounded-xl border border-stone-100">
        ...
        {related.length > 0 && (
          <section className="mt-20 pt-12 border-t border-stone-100" aria-labelledby="related-heading">
            <h2 id="related-heading" className="text-2xl font-semibold text-stone-900 mb-8" style={{ fontFamily: "'Playfair Display', serif" }}>You may also love</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {related.map((r) => (
                <a key={r.slug} href={`/products/${r.slug}`}
                  className="group flex gap-4 p-4 rounded-2xl border border-stone-100 hover:border-emerald-200 hover:shadow-md transition-all cursor-pointer">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-base font-semibold text-stone-900 group-hover:text-emerald-800 transition-colors truncate" style={{ fontFamily: "'Playfair Display', serif" }}>{r.name}</p>
