  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="relative pt-32 pb-24 overflow-hidden bg-stone-900">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="max-w-3xl">
            ...
            <h1 className="text-5xl sm:text-6xl font-semibold text-white mb-6 leading-tight" style={{ fontFamily: "'EB Garamond', serif" }}>
            ...
            <div className="flex flex-wrap gap-4">
              <a href="#apply" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-none bg-amber-500 text-stone-900 text-sm font-bold hover:bg-amber-400 transition-colors cursor-pointer" style={{ fontFamily: "Inter, sans-serif" }}>
                Apply now — it's free
              ...
              <a href="#how-it-works" className="inline-flex items-center gap-2 px-7 py-3.5 rounded-none border border-white/20 text-white/80 text-sm font-medium hover:border-white/50 hover:text-white transition-colors cursor-pointer" style={{ fontFamily: "Inter, sans-serif" }}>
                See how it works
              </a>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3, duration: 0.6 }}
            ...
            {[
              ...
              <div key={s.label}>
                <p className="text-3xl font-semibold text-amber-400 mb-1" style={{ fontFamily: "'EB Garamond', serif" }}>{s.val}</p>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {WHO.map((w, i) => (
              <motion.span key={w.label} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }}
                ...
                className="px-4 py-2 rounded-none bg-white border border-stone-200 text-stone-700 text-sm font-medium"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {BENEFITS.map((b, i) => (
              <motion.div key={b.title} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                <div className="w-11 h-11 rounded-none bg-amber-50 text-amber-700 flex items-center justify-center flex-shrink-0">
                  {b.icon}
                </div>
                <div>
                  <h3 className="text-base font-semibold text-stone-900 mb-1" style={{ fontFamily: "'EB Garamond', serif" }}>{b.title}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {STEPS.map((s, i) => (
              <motion.div key={s.num} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
                ...
                className="relative bg-white rounded-none p-8 border border-stone-100 shadow-sm">
                <span className="text-5xl font-bold text-stone-100 absolute top-6 right-6 select-none" style={{ fontFamily: "'EB Garamond', serif" }}>{s.num}</span>
          ...
          <AnimatePresence mode="wait">
            {step === "success" ? (
              <motion.div key="success" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
                ...
                <a href="/" className="inline-flex items-center gap-2 mt-8 px-6 py-3 rounded-none bg-stone-900 text-white text-sm font-medium hover:bg-stone-800 transition-colors cursor-pointer" style={{ fontFamily: "Inter, sans-serif" }}>
                  Back to Murall
                </a>
              </motion.div>
            ) : (
              <motion.form key="form" onSubmit={handleSubmit} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                className="space-y-5 bg-stone-50 rounded-none border border-stone-100 p-8">
                  ...
                  {[
                    ...
                    <div key={f.name}>
                      ...
                      <input name={f.name} value={(form as any)[f.name]} onChange={handleChange} placeholder={f.placeholder} required
                        className="w-full px-4 py-2.5 rounded-none border border-stone-200 bg-white text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all" style={{ fontFamily: "Inter, sans-serif" }} />
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select name="platform" value={form.platform} onChange={handleChange} required
                      className="w-full px-4 py-2.5 rounded-none border border-stone-200 bg-white text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all cursor-pointer" style={{ fontFamily: "Inter, sans-serif" }}>
                ...
                <div>
                  ...
                  <textarea name="why" value={form.why} onChange={handleChange} rows={3} placeholder="Tell us a bit about your audience and why Murall wallpaper is a good fit…"
                    className="w-full px-4 py-2.5 rounded-none border border-stone-200 bg-white text-sm text-stone-900 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition-all resize-none" style={{ fontFamily: "Inter, sans-serif" }} />
                </div>

                <button type="submit" disabled={loading}
                  className="w-full py-3.5 rounded-none bg-stone-900 text-white text-sm font-semibold hover:bg-stone-800 transition-all disabled:opacity-60 cursor-pointer flex items-center justify-center gap-2"
                  style={{ fontFamily: "Inter, sans-serif" }}>
                  {loading ? (
      ...
      <section className="py-20 bg-stone-50 border-t border-stone-100">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-semibold text-stone-900" style={{ fontFamily: "'EB Garamond', serif" }}>
          ...
          <div className="space-y-3">
            {FAQS.map((faq, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.07 }}
                className="bg-white rounded-none border border-stone-100 overflow-hidden">
                <button onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  ...
                  <span className="text-sm font-semibold text-stone-900" style={{ fontFamily: "'EB Garamond', serif" }}>{faq.q}</span>
      ...
      <section className="py-16 bg-stone-900 text-center">
        <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <h2 className="text-3xl font-semibold text-white mb-4" style={{ fontFamily: "'EB Garamond', serif" }}>Join 500+ Murall affiliates</h2>
